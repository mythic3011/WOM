...
function generateDropdownMenuItems(role) {
  const items = DROPDOWN_MENU_ITEMS[role] || [];
  return items
    .map(
      (item) => `
        <a
...
function renderUserSection(role, username) {
  if (role && username) {
    ...
    const menuItems = generateDropdownMenuItems(role);
    ...
    $("#userSection").html(`
      <div class="relative dropdown">
        ...
        >
          <div class="w-8 h-8 bg-white rounded-full flex items-center justify-center text-indigo-700 font-bold">
            ${initial}
          </div>
        ...
        >
          ...
          <button
            id="logoutBtn"
            class="w-full text-left px-4 py-2.5 text-sm text-red-600 hover:bg-red-50 transition-colors font-medium flex items-center group"
          >
...
function generateNavigationLinks(role) {
...
function renderMobileSidebar(role, username) {
  if (role && username) {
    const initial = getUserInitial(username);
    const links = generateNavigationLinks(role);
