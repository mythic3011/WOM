...
function getRoleConfig(role) {
  return ROLE_CONFIG[role] || { label: role, color: "bg-gray-600" };
}

function generateFooterLinks(role) {
...
function renderUserRoleBadge(role, username) {
...
export function initFooter() {
  ...
  if (role && username) {
    $("#userRoleBadge").html(renderUserRoleBadge(role, username));
    $("#quickLinks").html(generateFooterLinks(role));
    $("#logoutSection").html(renderLogoutButton());

    $(document)
      .off("click", "#footerLogoutBtn")
      .on("click", "#footerLogoutBtn", async function (e) {
        e.preventDefault();
        await handleLogout();
      });
  } else {
    $("#quickLinks").html(generateFooterLinks("guest"));
    $("#userRoleBadge").html("");
    $("#logoutSection").html("");
  }
}
