...
function renderHeaderStats(stats) {
  return stats
    ...
    <div class="text-center">
      ...
      <p class="text-lg font-bold ${stat.colorClass || "text-gray-900"}">${stat.value
        }</p>
    </div>
  `
    )
...
function renderTableContent({
  ...
  return `
    <table class="w-full" id="${tableId}">
      ...
      .map(
        (col) => `
            <th class="px-4 py-3 text-left text-xs font-semibold text-gray-600 uppercase tracking-wider ${col.sortable !== false && sortable
            ? "cursor-pointer hover:bg-gray-100 transition-colors"
            : ""
          } ${col.nowrap ? "whitespace-nowrap" : ""} ${col.headerClassName || ""
          }"
              data-column="${col.key}"
      ...
      .map(
        (row, index) => `
          <tr class="hover:bg-gray-50 transition-colors ${onRowClick ? "cursor-pointer" : ""
            ...
            .map(
              (col) => `
              <td class="px-4 py-4 text-sm ${col.cellClassName || ""} ${col.nowrap ? "whitespace-nowrap" : ""
                }">
  ...
  return `
    <div class="text-center py-16">
      <div class="inline-block">
        ...
        <h3 class="text-lg font-semibold text-gray-700 mb-2">${emptyState.title
    }</h3>
    ...
    <div class="px-6 py-4 border-t border-gray-200 bg-gray-50">
      <div class="flex flex-col sm:flex-row items-center justify-between gap-4">
        ...
        <nav class="flex items-center gap-2">
          <button
            class="pagination-btn px-3 py-2 text-sm font-medium rounded-lg border ${currentPage === 1
      ...
      .map(
        (page) => `
            <button
              class="pagination-btn px-3 py-2 text-sm font-medium rounded-lg border ${page === currentPage
      ...
      : ""
          ...
          <button
            class="pagination-btn px-3 py-2 text-sm font-medium rounded-lg border ${currentPage === totalPages
      ? "text-gray-400 bg-gray-100 border-gray-200 cursor-not-allowed"
      : "text-gray-700 bg-white border-gray-300 hover:bg-gray-50"
    }"
            data-page="${currentPage + 1}"
...
export function initTableFeatures(tableSelector, options = {}) {
  ...
  // Use event delegation with jQuery and event namespacing
  if (onSort) {
    $(document).on(
      "click.tableFeatures",
  ...
  if (onSelect) {
    $(document).on("change.tableFeatures", `${tableSelector} .table-select-all`, function () {
      const $selectAll = $(this);
      const isChecked = $selectAll.prop("checked");
      $(`${tableSelector} .table-row-select`).prop("checked", isChecked);

      const selectedIds = [];
      if (isChecked) {
        $(`${tableSelector} .table-row-select:checked`).each(function () {
    ...
    $(document).on("change.tableFeatures", `${tableSelector} .table-row-select`, function () {
      const selectedIds = [];
      $(`${tableSelector} .table-row-select:checked`).each(function () {
        selectedIds.push($(this).data("row-id"));
      });

      const totalRows = $(`${tableSelector} .table-row-select`).length;
      const checkedRows = selectedIds.length;
      $(`${tableSelector} .table-select-all`).prop(
        "checked",
  ...
  if (onRowClick) {
    $(document).on(
      ...
      function (e) {
        ...
        if (!$target.closest("input, button, a").length) {
  ...
  if (onSearch) {
    $(document).on("input.tableFeatures", `${tableSelector}-search`, function () {
...
export function createSimpleTable({
  ...
  return `
    <div class="overflow-x-auto">
      <table class="w-full ${striped ? "table-striped" : ""} ${hover ? "table-hover" : ""
    }">
        <thead class="bg-gray-50 border-b-2 border-gray-200">
          <tr>
            ${columns
      .map(
        (col) => `
              <th class="px-${compact ? "3" : "4"} py-${compact ? "2" : "3"
          } text-left text-xs font-semibold text-gray-600 uppercase ${col.nowrap ? "whitespace-nowrap" : ""
          }">
                ${col.label}
              </th>
            `
      ...
      ? data
        .map(
          (row, index) => `
              ...
              .map(
                (col) => `
                  <td class="px-${compact ? "3" : "4"} py-${compact ? "2" : "4"
