...
export class PerformanceWizardHandler {
    ...
    addEventHandler(selector, event, handler) {
        // Remove any existing handler for this selector/event combo first
    ...
    parseExistingPerformance(perf) {
        ...
        return {
            ...
            venueId: venueId,
            showtimes: showtimes,
            basePrice: perf.basePrice || perf.base_price,
            vipPrice: perf.vipPrice || perf.vip_price,
            premiumPrice: perf.premiumPrice || perf.premium_price,
            economyPrice: perf.economyPrice || perf.economy_price,
        };
    }
        ...
        try {
            switch (this.wizard.currentStep) {
                case 1:
                    ...
                    if (title.length) this.formData.title = title.val();
                    if (composer.length) this.formData.composer = composer.val();
                    if (conductor.length) this.formData.conductor = conductor.val();
                    if (description.length) this.formData.description = description.val();
                    if (duration.length) this.formData.duration = duration.val();
                    if (genre.length) this.formData.genre = genre.val();
                    break;
    ...
    async submit() {
        try {
            ...
            // Transform showtimes to the expected format
            const submitData = {
                ...this.formData,
